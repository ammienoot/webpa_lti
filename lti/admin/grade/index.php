  ...
  if ($resource_link->hasOutcomesService()) {
    ...
    if ($assessment_id && $marking_date) {
      ...
      if ($ok) {
        ...
        foreach($users as $user) {
          $sent++;
          $outcome = new LTI_Outcome($user->lti_result_sourcedid);
          if (isset($user_grades[$user->getId()])) {
            ...
            $err = !$resource_link->doOutcomesService(LTI_Resource_Link::EXT_WRITE, $outcome);
          } else {
            $err = !$resource_link->doOutcomesService(LTI_Resource_Link::EXT_DELETE, $outcome);
          }
          if ($err) {
            $errors++;
          }
