...
<?php
    ...
    foreach ($shares as $share) {
      ...
      echo "  <td class=\"icon\">";
      if ($can_edit) {
        echo "\n" . '    <a href="cancel.php?ci=' . urlencode($share->consumer_key) . '&rlid=' . urlencode($share->resource_link_id) . '" onclick="return confirm(\'Cancel share; are you sure?\');">';
        echo '<img src="../../../../images/buttons/cross.gif" width="16" height="16" alt="Cancel share" title="Cancel share" /></a>' . "\n";
      } else {
        echo '&nbsp;';
      }
      echo "  </td>\n";
      echo '  <td class="obj_info_text">' . $share->consumer_key . '</td>' . "\n";
      ...
      if ($can_edit) {
        if ($share->approved) {
          ...
          echo "onclick=\"return doApprove('{$i}', '" . APP__WWW . '/mod/' . LTI_MODULE_NAME . "/admin/share/approve.php', '{$share->consumer_key}', '{$share->resource_link_id}');\" />";
        } else {
